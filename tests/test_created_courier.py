...
class TestCreatedCourier:
    ...
    def test_success_create_courier(self, courier_data):
        created_courier = requests.post(f'{Urls.scooter_url}{Urls.created_courier}', json=courier_data) # Создаём нового курьера
    ...
    def test_double_create_courier(self, courier_data):
        created_courier = requests.post(f'{Urls.scooter_url}{Urls.created_courier}',json=courier_data)  # Создаём нового курьера
        assert created_courier.status_code == 201 # Проверяем создание нового курьера
        created_courier_2 = requests.post(f'{Urls.scooter_url}{Urls.created_courier}',json=courier_data)  # Пытаемся создать курьера с такими же данными повторно
